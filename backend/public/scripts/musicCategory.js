...
(async function getTracks() {
  const dataPromise = await fetch(`${window.location.origin}/music_data`);
  All_song = await dataPromise.json();
  ...
  for (let i = 0; i < All_song.length; i++) {
    let Html = ` <div class="song">
      <div class="img">
      <img src="${All_song[i].img}" alt="all"/>
      </div>
      <div class="more">
...
function play_song() {
  ...
  song_img.innerHTML = `<img src="${All_song[index_no].img}" alt="sng"/>`;
  playing_img.innerHTML = `<img src="${All_song[index_no].img}" alt="ply"/>`;
